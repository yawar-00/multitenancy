       ...
       <div class="overflow-hidden shadow-sm sm:rounded-lg" style="background-color:#bfc7a4">
        <div class="p-6 text-gray-900 dark:text-gray-100">
            <div class="d-flex align-items-center justify-content-between">
                ...
                <div class="card p-3 mb-3 shadow-sm rounded border-primary" style="width:50%; color:rgb(157, 159, 149)">
                    <h5 class="card-title mb-1">Products Limit</h5>
                    <div class="d-flex justify-content-evenly align-items-center mb-1">
                        <span  >You’ve used</span>
                        <strong id="used-count">{{ $product_used}}</strong>/<strong id="total-count">{{ $total_product }}</strong> products
                    </div>
                        @php
                            $used = $product_used;
                            $total = $total_product;
                        ...
                        <div class="progress" style="height: 20px; width:95%">
                            <div id="product-progress" class="progress-bar" role="progressbar"
                                    style="background-color:#00b894;width: {{ $percentage }}%;" aria-valuenow="{{ $used }}"
                                    aria-valuemin="0" aria-valuemax="{{ $total }}">
                                    
                            </div>
                        </div><span style="margin-left:5px">{{ $percentage }}%</span>
                        </div>
                </div>
            </div>
        <div id="message"></div>
...
// Wait for modal to fully hide, then clean up body styles
    ...
    function updateProductStats() {
        axios.get("{{url('/product-stats') }}")
            .then(function (response) {
                const data = response.data;
                document.getElementById('used-count').innerText = data.used;
                document.getElementById('total-count').innerText = data.total;

                const progressBar = document.getElementById('product-progress');
                progressBar.style.width = data.percentage + '%';
                progressBar.setAttribute('aria-valuenow', data.used);
